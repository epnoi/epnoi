...
try{
	...
	public void _putWikipediaPageAnnotatedContent(WikipediaPage wikipediaPage) {
		...
		if (sections.size() > 2) {
			...
			if (page.getTitle().equals("Apollo 11")){
			System.out.println("____________________> "+wikipediaPage);	
		
			}
			
			/*else{
			System.out.println("SECTIONS !!!! "+sections);
			System.out.println("SECTIONS CONTENT !!!! "+wikipediaPage.getSectionsContent().get(sections.get(0)));
			for (int i = sections.size() - 1; i >= 0; i--) {
				logger.info(" >> "+sections.get(i));
			}
			}*/
		}
	}
